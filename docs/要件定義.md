...
| :----------------------- | :--------------------------- | :----- | :------------------- | :------------------------------- |
| 蓄電池 (`027D01`)        | 瞬時充放電電力計測値         | `0xD3` | signed short (W)     | 正:充電, 負:放電                 |
| 蓄電池 (`027D01`)        | AC実効容量（充電）           | `0xA0` | unsigned long (Wh)   | 満充電時の容量                   |
| 蓄電池 (`027D01`)        | 蓄電残量3                    | `0xE4` | unsigned char (%)    |                                  |
| 住宅用太陽光発電 (`027901`) | 瞬時発電電力計測値           | `0xE0` | unsigned long (W)    |                                  |
| 分電盤メータリング (`028701`) | 瞬時電力計測値               | `0xC6` | signed long (W)      | ※EIBS7仕様に基づくEPC          |
| マルチ入力PCS (`02A501`)   | 瞬時電力計測値               | `0xE7` | signed short (W)     | ※EIBS7仕様に基づくEPC          |
...
| :------------------- | :----- | :---------------------------------------- | :----------------------------------- |
| 運転モード設定       | `0xDA` | unsigned char (`0x41`:自動, `0x42`:充電) | 他の値も存在するが、本システムでは主にこれらを使用 |
...
**2. 充電時間帯における制御**
   ...
   - **充電電力計算:**
     ...
     3. 目標充電電力 (W) = 目標充電量(Wh) * 60 / 残り時間(分) （ただし上限 5430W）
   - **充電電力設定:**
